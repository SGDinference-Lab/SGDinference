...
sgdi_lm_new = function(formula, 
  ...
  if (studentize){
    ...
    x_mean = apply(x[1:no_studentize,], 2, mean)
    x_sd = apply(x[1:no_studentize,], 2, sd)
  ...
  #----------------------------------------------
  out = sgdi_lm_cpp_new(x, y, burn, gamma_0, alpha, bt_start=bt_t, inference=inference, rss_idx=rss_idx, x_mean=x_mean_in, x_sd=x_sd_in)
